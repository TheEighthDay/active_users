...
def vec(r: list, l: list, c: list, a: list, author_id: list):
    """
    ...
    author_num = len(author_deduplicate)  # 所有行为涉及的作者的个数
    ...
    return np.array([
        ...
        # action_sd,
        action_list_len]
      + page_cnt_list.tolist() 
      # + page_per_d.tolist() 
      # + page_per_l.tolist() 
      # + [page_sd]
      + type_cnt_list.tolist()
      # + type_per_d.tolist() 
      # + type_per_l.tolist() 
      + [
        # type_sd,
...
def gen_vec_one_day():
    ...
    train_y=[]
    for i in range(1,2):
        userids=r[r.register_day == i]['user_id'].get_values()
        # print(userids)
        # print (userids.shape)
        result = []
        author_id = list(a[a.day==i]['author_id'].get_values())
        for userid in tqdm(userids):
            r_temp=r[r.user_id==userid].get_values()[0]
            l_temp=l[l.user_id==userid].get_values()
            c_temp=c[c.user_id==userid].get_values()
            a_temp=a[a.user_id==userid].get_values()
            result.append(vec(r_temp, l_temp, c_temp, a_temp, author_id))
            
        fulture_id=l[l.user_id==userids][l.day==[x+i+1 for x in range(7)]]['user_id'].drop_duplicates().get_values()
        ending=[ 1 if userid in fulture_id else 0 for userid in userids ]
        train_X.extend(result)
        train_y.extend(ending)
